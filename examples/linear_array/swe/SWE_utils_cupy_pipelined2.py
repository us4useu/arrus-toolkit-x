...
class ShearwaveDetection(Operation):
    ...
    def prepare(self, const_metadata: arrus.metadata.ConstMetadata):
        ...
        self.c  = const_metadata.context.sequence.ops[0].tx.speed_of_sound
        self.frame_pri = const_metadata.context.sequence.ops[0].pri * 2
...
class ShearwaveMotionDataFiltering(Operation):
    ...
    def prepare(self, const_metadata: arrus.metadata.ConstMetadata):
        ...
        self.fs = 1/(const_metadata.context.sequence.ops[0].pri * 2)
...
class Mask_SWS_Maps(Operation):
    # Input: 5D data: [dataset, SWV/r, LR/RL, z, x], or 4D data [SWV/r, LR/RL, z, x]
    # Output: the same dimensions
    def __init__(self, LR_masks, RL_masks, num_pkg=None, filter_pkg=None, **kwargs):
        # LR_masks: a list of indices [start, end] for each subsequent LR map, for example, [[30, 200], [60, 200]]. Length of list should match input data number of datasets 
        # RL_masks: a list of indices [start, end] for each subsequent RL map, for example, [[0, 100], [0, 50]]. Length of list should match input data number of datasets 
        self.xp = num_pkg
        self.filter_pkg = filter_pkg
        self.kwargs = kwargs 

        self.LR_masks = LR_masks
        self.RL_masks = RL_masks
        self.abort_processing = 0
        self.n_datasets = 0
    ...
    def prepare(self, const_metadata: arrus.metadata.ConstMetadata):
        ...
        # Check the masks
        if(len(self.LR_masks) != len(self.RL_masks)):
            print("LR and RL masks number not equal, aborting masking!")
            self.abort_processing = 1

        if(len(input_shape) == 4): # data from a single acquisition (4D array)
            self.n_datasets = 1
            z_dim = input_shape[2]
            x_dim = input_shape[3]
            if(len(self.LR_masks) != 1):
                print("Wrong number of masks, aborting masking!")
                self.abort_processing = 1

            if(len(self.RL_masks) != 1):
                print("Wrong number of masks, aborting masking!")
                self.abort_processing = 1
        ...
        elif(len(input_shape) == 5): # data from multiple acquisitions (4D array)
            self.n_datasets = input_shape[0]
            z_dim = input_shape[3]
            x_dim = input_shape[4]
            
            if(len(RL_masks) != input_shape[0]):
                print("Wrong number of masks, aborting masking!")
                self.abort_processing = 1
            elif(len(LR_masks) != input_shape[0]):
                print("Wrong number of masks, aborting masking!")
                self.abort_processing = 1            
            
            else:
                self.abort_processing = 0 
                self.n_masks = len(self.RL_masks)
    
        for i in range(self.n_masks):
            if(self.LR_masks[i][0]<0):
                self.LR_masks[i][0] = 0
            if(self.RL_masks[i][0]<0):
                self.RL_masks[i][0] = 0
        
        # Metadata - update output dimensions
        return const_metadata.copy(input_shape=input_shape)
    ...
    def process(self, data):
        if(self.abort_processing == 0):
            masked_data = data.copy()
            if(self.n_datasets == 1):
                masked_data[1, 0, :, self.LR_masks[0][0]:self.LR_masks[0][1]] = 0
                masked_data[1, 1, :, self.RL_masks[0][0]:self.RL_masks[0][1]] = 0

            else:
                for i in range(self.n_datasets):
                    masked_data[i, 1, 0, :, self.LR_masks[i][0]:self.LR_masks[i][1]] = 0
                    masked_data[i, 1, 1, :, self.RL_masks[i][0]:self.RL_masks[i][1]] = 0
...
class SWS_Datasets_Compounding(Operation):
    # Input data format: 5D data: [dataset, SWV/r, LR/RL, z, x]
    # Output data format: 3D data: 2x 2D array: SWS: 3D array [z, x], SWS_r: 3D array [z, x]. Arrays concat'd along axis=0. For example: [SWV/r, z, x]
    ...
    def prepare(self, const_metadata: arrus.metadata.ConstMetadata):
        ## Metadata -update output dimensions
        input_shape  = const_metadata.input_shape
        output_shape = (2, input_shape[3], input_shape[4])

        self.x_dim = input_shape[0]
        self.z_dim = input_shape[1]
        self.n_datasets = input_shape[4]
    ...
    def process(self, data):
        ...
        SWS   = cp.reshape(SWS, (self.n_datasets*2, self.z_dim, self.x_dim))
        SWS_r = cp.reshape(SWS_r, (self.n_datasets*2, self.z_dim, self.x_dim))
        ...
        # Pack data
        SWSc  = SWSc[cp.newaxis, ...]
        SWScr = SWScr[cp.newaxis, ...]
        return cp.concatenate((SWSc, SWScr), axis=0) 


class SWS_Compounding(Operation):
    # Input data format: 4D data: 2x 3D array: SWS: 3D array [dataset, z, x], SWS_r: 3D array [dataset, z, x]. Arrays concat'd along axis=0. For example: [SWV/r, LR/RL, z, x]
    def __init__(self, num_pkg=None, filter_pkg=None, **kwargs):
        self.xp = num_pkg
        self.filter_pkg = filter_pkg
        self.kwargs = kwargs 
