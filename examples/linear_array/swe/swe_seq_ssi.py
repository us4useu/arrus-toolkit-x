...
def main():
    ...
    id               = 1    # id of the file to store collected RF data
    
    # Pushing sequence parameters
    push_hv          = 30                                                    # Push pulse HV voltage [V] 
    push_freq        = np.array([130.0/20*1e6, 130.0/20*1e6, 130.0/20*1e6])  # Push pulse center frequencies [Hz]
    push_length      = np.array([120e-6, 150e-6, 200e-6])                    # Push pulse durations [s]
    push_txelements  = np.array([21, 41, 62])                                # Push pulse aperture sizes [# of probe elements]
    push_focus       = np.array([10e-3, 20e-3, 30e-3])                       # Push pulse focus depth [m]
    push_center      = np.array([31.5, 95.5, 159.5])                          # Push pulse aperture center elements indices (for each lateral position of SSI beam) 
    ...
    n_samples     = 4864              # Number of samples to be collected in each RX operation of tracking sequence. Must be multiple of 64
    imaging_pw_n_repeats = 80         # Number of frames to be captured in tracking sequence

    # SSI sequence parameters
    ssi_sri       = 1000e-3            # Sequence repetition interval [s]. Means interval between {push + imaging} sequences afor consecutive lateral positions.
    ...
    with arrus.Session(protofile, medium=medium) as sess:
        ...
        for k in range(len(push_center)):     # sweep through lateral positions of the beams
            for i in range(len(push_focus)):  # sweep through axial focus positions
                ...
                push_sequence[k][i] = TxRx(
                        Tx(aperture=arrus.ops.us4r.Aperture(center_element=push_center[k], size=push_txelements[i]), 
        ...
        seq0 = TxRxSequence(ops=push_sequence[0]+imaging_sequence, sri=ssi_sri, name="Seq0") # each lateral SSI push has its own sequence
        seq1 = TxRxSequence(ops=push_sequence[1]+imaging_sequence, sri=ssi_sri, name="Seq1") 
        seq2 = TxRxSequence(ops=push_sequence[2]+imaging_sequence, sri=ssi_sri, name="Seq2") 
