...
def main(pwiFreq, pwiCycles, pwiPri, idx):
    
    ############## PARAMETERS #############################################
    path          = './Parametric_tests/Test_2/i/'
    acq_type      = 'ssi'           # Acquisition type: {'ssi', 'cuse', 'cpwi'}

    ## ACQUISITION PARAMETERS #####
    # Push parameters
    pb_tx_freq    = [4.4] #[4.4, 4.4, 4.4]  # Push beam frequency [MHz]
    pb_tx_length  = [250] #[150, 150, 150]  # Push beam duration [us]
    ...
    pwi_pri       = pwiPri           # PWI PRI [us] - it is only for half RX aperture! 
    pwi_tx_angles = [-4, 0, 4]       # Angles for CPWC
    pwi_tx_freq   = pwiFreq * 1e6            # PWI pulses frequency [Hz]
    pwi_ncycles   = pwiCycles                # PWI pulses number of cycles
    ...
    # RF Filtering
    rf_filter_band        = [3.8e6, 7e6]  # Desired pass band, [Hz]
    ...
    swd_ensemble_length   = 6              # Averaging slow-time axis kernel size
    ...
    swse_x_range       = [[20, 300], [0, 10]]  # Range within x-dimension to perform SWS estimation (LR and RL, repsectively)
    ...
    with arrus.Session("./us4r.prototxt") as sess:
        ...
        if(txpb_issue == 0):
            ...
            # Save data
            scipy.io.savemat(path + 'sws'   + '_freq' + str(pwiFreq) + '_cycles' + str(pwiCycles) + '_pri' + str(pwiPri) + '_id_' + str(idx) +'.mat', dict(data=sws)) 
            scipy.io.savemat(path + 'energy_swd'   + '_freq' + str(pwiFreq) + '_cycles' + str(pwiCycles) + '_pri' + str(pwiPri) + '_id_' + str(idx) +'.mat', dict(data=E1)) 
            scipy.io.savemat(path + 'energy_swdf'  + '_freq' + str(pwiFreq) + '_cycles' + str(pwiCycles) + '_pri' + str(pwiPri) + '_id_' + str(idx) +'.mat', dict(data=E2)) 

            scipy.io.savemat(path + 'swd'    + '_freq' + str(pwiFreq) + '_cycles' + str(pwiCycles) + '_pri' + str(pwiPri) + '_id_' + str(idx) +'.mat', dict(data=swd)) 
            scipy.io.savemat(path + 'swdf'   + '_freq' + str(pwiFreq) + '_cycles' + str(pwiCycles) + '_pri' + str(pwiPri) + '_id_' + str(idx) +'.mat', dict(data=swdf)) 
            scipy.io.savemat(path + 'bmode'  + '_freq' + str(pwiFreq) + '_cycles' + str(pwiCycles) + '_pri' + str(pwiPri) + '_id_' + str(idx) +'.mat', dict(data=Bmode)) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pwiFreq", dest="pwiFreq", type=float)
    parser.add_argument("--pwiCycles", dest="pwiCycles", type=int)
    parser.add_argument("--pwiPri", dest="pwiPri", type=float)
    parser.add_argument("--idx", dest="idx", type=int)
    args = parser.parse_args()
    args = main(args.pwiFreq, args.pwiCycles, args.pwiPri, args.idx)  
