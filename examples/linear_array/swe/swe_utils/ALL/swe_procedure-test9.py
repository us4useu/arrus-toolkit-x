...
def main(pbLen, hv, idx):
    
    ############## PARAMETERS #############################################
    path          = './Parametric_tests/Test_11/'
    acq_type      = 'cuse'           # Acquisition type: {'ssi', 'cuse', 'cpwi'}

    ## ACQUISITION PARAMETERS #####
    # Push parameters
    pb_tx_freq    = [4.4] #[4.4, 4.4, 4.4]  # Push beam frequency [MHz]
    pb_tx_length  = [200] #[150, 150, 150]  # Push beam duration [us]
    pb_focus_z    = [30]  #[20, 25, 30]     # Push beam focus [mm] - each beam is focused at the same depth. Specify very big number for plane wave
    pb_aperture   = [42]  #[32, 42, 51]     # Push beam aperture [# of elements of each beam] - must be even
    pb_cuse_beams = [3]   #[2, 2, 2]        # Number of comb beams. Always spaced to for symmetric comb. Relevant only for 'cuse' acq type.
    pb_cuse_sep   = [42]     # Comb beams separation [# of elements between beam centers].  Relevant only for 'cuse' acq type.
    ...
    with arrus.Session("./us4r.prototxt") as sess:
        ...
        if(txpb_issue == 0):
            ...
            Eswd = np.power(ddataE, 2)
            Eswd = np.squeeze(np.sum(Eswd, 2))
            
            # Get energy map from swdf data
            ddataE1 = cp.asarray(np.squeeze(swdf[0, 10:-20, ...]))
            ddataE2 = cp.asarray(np.squeeze(swdf[1, 10:-20, ...]))
            ...
            # Save data
            scipy.io.savemat(path + 'sws' + '_pblen' + str(pbLen) + '_hv' + str(hv) + '_id_' + str(idx) +'.mat', dict(data=sws))
            scipy.io.savemat(path + 'energy_swd' + '_pblen' + str(pbLen) + '_hv' + str(hv)+ '_id_' + str(idx) +'.mat', dict(data=Eswd)) 
            scipy.io.savemat(path + 'energy_swdf' + '_pblen' + str(pbLen)+ '_hv' + str(hv) + '_id_' + str(idx) +'.mat', dict(data=E_cpu)) 
            scipy.io.savemat(path + 'swd' + '_pblen' + str(pbLen)+ '_hv' + str(hv) + '_id_' + str(idx) +'.mat', dict(data=swd)) 
            scipy.io.savemat(path + 'swdf' + '_pblen' + str(pbLen)+ '_hv' + str(hv) + '_id_' + str(idx) +'.mat', dict(data=swdf)) 
            scipy.io.savemat(path + 'Bmode' + '_pblen' + str(pbLen)+ '_hv' + str(hv) + '_id_' + str(idx) +'.mat', dict(data=Bmode)) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hv", dest="hv", type=int)
    parser.add_argument("--idx", dest="idx", type=int)
    args = parser.parse_args()
    args = main(args.pbLen, args.hv, args.idx)  
