...
pb_tx_length  = [10] #[150, 150, 150]  # Push beam duration [us]
pb_aperture   = [47]  #[32, 42, 51]     # Push beam aperture [# of elements of each beam] - must be even
...
pwi_frames    = 10              # PWI frames to capture
pwi_pri       = 400              # PWI PRI [us] - it is only for half RX aperture! 
...
swse_frames        = [0, 5]                # Range of frames to be used for SWS estimation
...
rx_conf +=  [(n_samples, pwi_pri)] # PUSH TX/RX
...
with arrus.Session("./us4r.prototxt") as sess:
    ...
    if(txpb_issue == 0):
        ...
        swdf  = cpu_data[2]
        swd   = cpu_data[3]
        scipy.io.savemat(f"Datasets/Bmode.mat", dict(data=Bmode))
        scipy.io.savemat(f"Datasets/sws.mat", dict(data=sws))
        scipy.io.savemat(f"Datasets/swd.mat", dict(data=swd))
        #scipy.io.savemat(f"Datasets/swdf.mat", dict(data=swdf))
        print("Acquired data saved to drive.")  
