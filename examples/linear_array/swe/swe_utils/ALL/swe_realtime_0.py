...
pb_tx_length  = [150] #[150, 150, 150]  # Push beam duration [us]
pb_aperture   = [58]  #[32, 42, 51]     # Push beam aperture [# of elements of each beam] - must be even
...
df_sws_range = [0.5, 5.0]              # SWS filtering pass band [m/s]
...
swse_SWV_range     = [0.5, 5.0]             # Range of SWS of interest. All higher or lower will be clipped.
...
# Post-processing
median_filter_size = 9    # Median filter kernel size. Kernel is a square.
...
with arrus.Session("./us4r.prototxt") as sess:
    ...
    if(txpb_issue == 0):
        ...
        for i in range(5):
            ...
            swdf = np.squeeze(swdf[0, ...] + swdf[1, ...])

            # Print first frames  
            if(i == 0): # only at first run
                axim0 = ax0.imshow(data, cmap='gray', norm=norm_bmode) 
                axim1 = ax1.imshow(swdf[:, :, 0], cmap='jet', norm=norm_swd) 
                axim2 = ax2.imshow(sws[0], cmap='jet', norm=norm_sws) 
                axim3 = ax3.imshow(sws[1], cmap='jet', norm=norm_r)
                
                ax0.set_title('Bmode [dB]')
                ax1.set_title('SWD [m/s]')
                ax2.set_title('SWS [m/s]')
                ax3.set_title('r [0..1]')

            axim0.set_data(data)
            axim2.set_data(sws[0])
            axim3.set_data(sws[1])
            
            for j in range(90):
                axim1.set_data(swdf[:, :, j])
                # Update image
                #fig.canvas.draw()
                fig.canvas.flush_events()
                #time.sleep(0.01)
