...
def main(idx):
    ...
    ############## PARAMETERS #############################################
    path          = './Parametric_tests/Test_12/'
    acq_type      = 'ssi'           # Acquisition type: {'ssi', 'cuse', 'cpwi'}

    ## ACQUISITION PARAMETERS #####
    # Push parameters
    pb_tx_freq    = [4.4, 4.4, 4.4] #[4.4, 4.4, 4.4]  # Push beam frequency [MHz]
    pb_tx_length  = [200, 200, 200] #[150, 150, 150]  # Push beam duration [us]
    pb_focus_z    = [15, 30, 40]  #[20, 25, 30]     # Push beam focus [mm] - each beam is focused at the same depth. Specify very big number for plane wave
    pb_aperture   = [24, 50, 66]  #[32, 42, 51]     # Push beam aperture [# of elements of each beam] - must be even
    pb_cuse_beams = [1, 1, 1]   #[2, 2, 2]        # Number of comb beams. Always spaced to for symmetric comb. Relevant only for 'cuse' acq type.
        ...
        if txpb.open():
            ...
            if(acq_type == 'ssi'):
                ...
                for i in range(3):
                    ...
                    Bmode = cpu_data[0]
                    sws   = cpu_data[1]
                    scipy.io.savemat(path + 'sws' +'_id_' + str(idx) + '_shift_' + str(pb_shift[i]) + '_B.mat', dict(data=sws))
                    scipy.io.savemat(path + 'bmode' +'_id_' + str(idx) + '_shift_' + str(pb_shift[i]) + '_B.mat', dict(data=Bmode))
