...
def main(idx):
    ...
    pb_focus_z    = [25, 40]  #[20, 25, 30]     # Push beam focus [mm] - each beam is focused at the same depth. Specify very big number for plane wave
    pb_aperture   = [42, 66]  #[32, 42, 51]     # Push beam aperture [# of elements of each beam] - must be even
    pb_cuse_beams = [1, 1]   #[2, 2, 2]        # Number of comb beams. Always spaced to for symmetric comb. Relevant only for 'cuse' acq type.
        ...
        if txpb.open():
            ...
            if(acq_type == 'ssi'):
                ...
                for i in range(3):
                    ...
                    print('Firing with shift: ' + str(pb_shift[i]))
                    ...
                    sws   = cpu_data[1]
                    rf    = cpu_data[2] 
                    scipy.io.savemat(path + 'sws' +'_id_' + str(idx) + '_shift_' + names[i] + '.mat', dict(data=sws))
                    scipy.io.savemat(path + 'bmode' +'_id_' + str(idx) + '_shift_' + names[i] + '.mat', dict(data=Bmode))
                    scipy.io.savemat(path + 'rf' +'_id_' + str(idx) + '_shift_' + names[i] + '.mat', dict(data=rf))
