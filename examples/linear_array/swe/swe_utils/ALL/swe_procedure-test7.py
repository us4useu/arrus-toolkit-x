...
def main(pbFoci, pbAp, pbApodScale):
    
    ############## PARAMETERS #############################################
    path          = './Parametric_tests/Test_7/'
    acq_type      = 'pSWE_apod'           # Acquisition type: {'ssi', 'cuse', 'cpwi'}
    ...
    with arrus.Session("./us4r.prototxt") as sess:
        ...
        if txpb.open():
            ...
            if(acq_type == 'cpwi'):
                txpb.configure_CPWI_procedure(pwi_tx_freq = pwi_tx_freq/1e6, \
                                              ...
                                              c = c, \
                                              fs = fs/1e6)            
            elif(acq_type == 'pSWE_apod'):
                txpb.configure_SWE_pSWE_Apod_procedure(pb_tx_freq = 4.4,
                                                       pb_tx_length = pb_tx_length[0],
                                                       pb_aperture = 48, #pb_aperture[0],
                                                       pb_focus_z = pb_focus_z[0], 
                                                       pb_shift = 0, 
                                                       pb_apod_scale = pbApodScale,
        ...
        if(txpb_issue == 0):
            ...
            # Save data
            scipy.io.savemat(path + 'sws' + '_pbAp' + str(pbAp) + '_foc' + str(pbFoci) + '_s' + str(pbApodScale) +'.mat', dict(data=sws))
            scipy.io.savemat(path + 'energy_swd' + '_pbAp' + str(pbAp) + '_foc' + str(pbFoci) + '_s' + str(pbApodScale) +'.mat', dict(data=Eswd)) 
            scipy.io.savemat(path + 'energy_swdf' + '_pbAp' + str(pbAp) + '_foc' + str(pbFoci) + '_s' + str(pbApodScale) +'.mat', dict(data=E_cpu)) 
            scipy.io.savemat(path + 'swd'+'_pbAp' + str(pbAp) + '_foc' + str(pbFoci) + '_s' + str(pbApodScale) +'.mat', dict(data=swd)) 
            scipy.io.savemat(path + 'swdf'+'_pbAp' + str(pbAp) + '_foc' + str(pbFoci) + '_s' + str(pbApodScale) +'.mat', dict(data=swdf)) 
            scipy.io.savemat(path + 'Bmode'+'_pbAp' + str(pbAp) + '_foc' + str(pbFoci) + '_s' + str(pbApodScale) +'.mat', dict(data=Bmode)) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pbApodScale", dest="pbApodScale", type=float)
    args = parser.parse_args()
    args = main(args.pbAp, args.pbFoci, args.pbApodScale)  
