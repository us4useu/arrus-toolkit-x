...
def main(idx):
    ...
    ############## PARAMETERS #############################################
    path          = './Parametric_tests/Test_10/'
    acq_type      = 'ssi'           # Acquisition type: {'ssi', 'cuse', 'cpwi'}

    ## ACQUISITION PARAMETERS #####
    # Push parameters
    pb_tx_freq    = [4.4, 4.4] #[4.4, 4.4, 4.4]  # Push beam frequency [MHz]
    pb_tx_length  = [300, 300] #[150, 150, 150]  # Push beam duration [us]
    pb_focus_z    = [25, 35]  #[20, 25, 30]     # Push beam focus [mm] - each beam is focused at the same depth. Specify very big number for plane wave
    pb_aperture   = [48, 66]  #[32, 42, 51]     # Push beam aperture [# of elements of each beam] - must be even
    ...
    swse_x_range       = [[0, 300], [0, 300]]  # Range within x-dimension to perform SWS estimation (LR and RL, repsectively)
    ...
    with arrus.Session("./us4r.prototxt") as sess:
        ...
        if(txpb_issue == 0):
            ...
            # Save data
            scipy.io.savemat(path + 'sws' +'_id_' + str(idx) +'.mat', dict(data=sws))
            scipy.io.savemat(path + 'energy_swd' + '_id_' + str(idx) +'.mat', dict(data=Eswd)) 
            scipy.io.savemat(path + 'energy_swdf' + '_id_' + str(idx) +'.mat', dict(data=E_cpu)) 
            scipy.io.savemat(path + 'swd' + '_id_' + str(idx) +'.mat', dict(data=swd)) 
            scipy.io.savemat(path + 'swdf' + '_id_' + str(idx) +'.mat', dict(data=swdf)) 
            scipy.io.savemat(path + 'Bmode' + '_id_' + str(idx) +'.mat', dict(data=Bmode)) 
